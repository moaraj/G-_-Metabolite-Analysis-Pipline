...
metab_data4 <- read.delim(file = "3.csv_moaraj4.names.csv", sep = ",")
...
```{r HMDB Loop UP function}
HMDB_Lookup <- function(metabolite.data, nfactorcols){
  
  require(XML)
  
  metabolite.names <- colnames(metabolite.data)[(nfactorcols + 1):ncol(metabolite.data)]
...
```{r HMDB Id Conver}
convert.ids <- function(data, nfactorcols){
  
  match.df <- HMDB_Lookup(data, nfactorcols)
...
Generate Data for Basic Set comparisons of Metabolites
```{r}
metab_data0.names <- convert.ids(metab_data0,10)
metab_data1.names <- convert.ids(metab_data1,10)
metab_data3.names <- convert.ids(metab_data3,10)
metab_data4.names <- convert.ids(metab_data4,11)
...
metab_data0.hmdb <- remove.bad.metabs(metab_data0.names)
metab_data1.hmdb <- remove.bad.metabs(metab_data1.names)
metab_data3.hmdb <- remove.bad.metabs(metab_data3.names)
metab_data4.hmdb <- remove.bad.metabs(metab_data4.names)
```
...
```{r}
    Diet.DF.0 <- DietFoldChange(metab_data0.names, 9)
    Diet.DF.1 <- DietFoldChange(metab_data1.names, 9)
...
Age.DF <- Age.DF %>% mutate(AbsDiff = Young.mean - Old.mean) %>%
                       ...
                       mutate(Std.FoldChange = (Young.mean/Young.var) / (Old.mean/Old.var)) %>%
...
```{r]
    ...
    Age.DF.0 <- AgeFoldChange(metab_data0.names, 9)
    Age.DF.1 <- AgeFoldChange(metab_data1.names, 9)
    Age.DF.3 <- AgeFoldChange(metab_data3.names, 13)
    Age.DF.4 <- AgeFoldChange(metab_data4.names, 11)


    plot(Diet.DF.0$Young.mean, Diet.DF.0$Old.mean, log = "xy")

    model.Diet = lm(data = Age.DF.0, formula = Age.DF.0$Old.mean ~ Age.DF.0$Young.mean)
    summary(model.Diet)
    plot(model.Diet,1)
...
#Perform the Same analysis on the Age Cohorts
```{r}
...
volcano.plot <- function(data){
  ggplot(data = data, 
  aes(x = FoldChange.Log2, y = -log(T.Test), color = -log(T.Test) > 3)) + geom_point() + 
  scale_y_continuous(limits = c(-1,25)) + scale_x_continuous(limits = c(-4,2.5)) +
  geom_vline(xintercept = 0) + geom_hline(yintercept = 0) +
  ggrepel::geom_label_repel( aes(label=ifelse(-log(T.Test) > 3,as.character(Metabolite),''),
  fontface = 'bold', color = 'white')) + theme_minimal(base_size = 16)
}


volcano.plot(Diet.DF.0)
volcano.plot(Diet.DF.1)
volcano.plot(Diet.DF.3)
volcano.plot(Diet.DF.4)
